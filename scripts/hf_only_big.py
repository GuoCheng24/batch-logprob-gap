"""Same protocol as hf_only.py; only the fp32 arm is sharded across GPUs so 7B fits.
...
R = {}
def _load(dt):
    """bf16/fp16 fit one 24GB card; fp32 (~30GB for 7B) is sharded across the visible GPUs.
    Inputs still go to cuda:0; accelerate's hooks move activations between shards."""
    if dt == torch.float32 and torch.cuda.device_count() > 1:
        n = torch.cuda.device_count()
        return AutoModelForCausalLM.from_pretrained(
            MODEL, dtype=dt, device_map="auto",
            max_memory={i: "20GiB" for i in range(n)}).eval()
    return AutoModelForCausalLM.from_pretrained(MODEL, dtype=dt).to("cuda:0").eval()
