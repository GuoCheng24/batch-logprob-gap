...
set -euo pipefail
cd "$(dirname "$0")"
GPUS=${1:?GPU list, e.g. 0,1}; ARM=${2:?arm}
...
case "$ARM" in
  nocorr_tp08) bash gate.sh 0.8 long_nocorr_tp08 $COMMON ;;
  geo_tp08)    bash gate.sh 0.8 long_geo_tp08 $GEO $COMMON ;;
  geofix_tp08) PYTHONPATH=${VERL_PATCHED:?path to patched verl} bash gate.sh 0.8 long_geofix_tp08 $GEO $FIX $COMMON ;;
  geo_tp10)    bash gate.sh 1.0 long_geo_tp10 $GEO $COMMON ;;
  seqmis_tp08) bash gate.sh 0.8 long_seqmis_tp08 $SEQMIS $COMMON ;;
