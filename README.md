...
only the shape of the tensor around them changed. On four architectures the resulting
importance ratio `exp(logp_new - logp_old)` leaves `[0.9, 1.1]` for **1.8% to 36.9%** of tokens,
...
| model | bf16 b1 vs b8 | fp16 | fp32 | bf16 rerun, same batch | bf16 b1 vs b32 |
|---|---|---|---|---|---|
| pythia-410m (NeoX) | 36.70% | 2.49% | 0.00% | **0.00%** | 36.65% |
...
Percentage of scored tokens whose importance ratio falls outside `[0.9, 1.1]`, 4096 tokens per
...
| model | A padded ragged | B equal length, no padding | C same sequence x8 vs alone | D batch 8, regrouped |
|---|---|---|---|---|
| pythia-410m | 36.87% | 36.23% | 18.75% | 0.00% |
| Qwen2.5-0.5B | 9.74% | 10.06% | 6.25% | 0.00% |
| Granite MoE | 14.09% | 14.21% | 20.31% | **12.60%** |
...
- **One GPU generation.** Everything is RTX 4090, SM89. Reduction strategies differ across
  architectures.
- **Small models.** The largest here is 4B; the production report was a 30B MoE. Note that 4B
  shows the *lowest* rate in this set, so scale may attenuate it -- this harness cannot say.
